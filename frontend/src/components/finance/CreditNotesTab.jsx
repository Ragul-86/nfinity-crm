...
const STATUS_CONFIG = {
  ...
  issued:    { label: 'Issued',    color: 'bg-blue-500/10 text-blue-400' },
  applied:   { label: 'Applied',   color: 'bg-green-500/10 text-green-400' },
  cancelled: { label: 'Cancelled', color: 'bg-muted text-muted-foreground' },
}

function CreditNoteModal({ open, onClose, doc, onSaved }) {
  ...
  const onSubmit = async (data) => {
    try {
      if (isEdit) await api.put(`/finance/credit-notes/${doc._id}`, data)
      else        await api.post('/finance/credit-notes', data)
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md" aria-describedby={undefined}>
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Edit Credit Note' : 'New Credit Note'}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label>Reason *</Label>
            <Input {...register('reason', { required: true })} placeholder="Reason for credit note" />
          </div>
          <div className="space-y-1.5">
            <Label>Notes</Label>
...
export default function CreditNotesTab() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/finance/credit-notes', { params: { page, limit: 20 } }).then(r => r.data),
    refetchInterval: 30_000,
  })

  const invalidate = () => qc.invalidateQueries(['finance-credit-notes'])

  const deleteMut = useMutation({
    mutationFn: id => api.delete(`/finance/credit-notes/${id}`),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        {totalIssued > 0 && (
          <div className="bg-card border border-border rounded-xl px-4 py-2 text-sm">
            Total issued: <strong className="text-blue-400">{fmt(totalIssued)}</strong>
          </div>
        )}
        <Button size="sm" className="ml-auto" onClick={() => { setEditDoc(null); setShowForm(true) }}>
          <Plus className="w-3.5 h-3.5 mr-1.5" />New Credit Note
        </Button>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="px-3 py-3 text-left text-xs font-medium text-muted-foreground">Credit Note #</th>
                ...
                {notes.map(n => {
                  ...
                  return (
                    <tr key={n._id} className="hover:bg-muted/20 transition-colors">
                      <td className="px-3 py-3 font-medium text-primary">{n.creditNoteNumber}</td>
                      ...
                      <td className="px-3 py-3 text-right font-semibold text-blue-400">{fmt(n.amount)}</td>
      ...
      <CreditNoteModal open={showForm} onClose={() => { setShowForm(false); setEditDoc(null) }}
